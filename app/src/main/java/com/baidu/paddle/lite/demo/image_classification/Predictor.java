...
public class Predictor {
    ...
    protected boolean loadModel(Context appCtx, String modelPath, int cpuThreadNum, String cpuPowerMode) {
        ...
        config.setModelFromFile(realPath + File.separator + "mobilenetv3_paddlelite2.nb");
    ...
    public void postProcess() {
        ...
        float[] a = outputTensor.getFloatData();
        ...
        if (wordLabels.size() > 0) {
            ...
            for (int i = 0; i < 3;i++)
            {
                results[i] = max_index[i] != 41 ? Float.parseFloat(wordLabels.get(max_index[i])) : (float) 0.0;
                if (results[i] < min){
                    min = results[i];
                    minLabels = wordLabels.get(max_index[i]);
                }
            }
            if (min == 0)
            {
                top1Result = "推理结果: 输入图片非规定耳标格式，请按设定格式输入";
            }
            else {
                top1Result = "推理结果: " + minLabels + "℃";
            }
